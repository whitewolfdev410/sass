...
const TimerController = (props: any) => {
	const [renderedStreamDuration, setRenderedStreamDuration] =
			useState<any>("00:00:00"),
		streamDuration = useRef(0),
		previousTime = useRef(0),
		requestAnimationFrameId = useRef<any>(null),
		[isStartTimer, setIsStartTimer] = useState<boolean>(false),
		[isStopTimer, setIsStopTimer] = useState<boolean>(false),
		[isPauseTimer, setIsPauseTimer] = useState<boolean>(false),
		[isResumeTimer, setIsResumeTimer] = useState<boolean>(false),
...
function AudioRecord() {
	...
	function Deletedata(id: any) {
		const data = audios.filter((e: any, index: any) => {
			return index !== id;
		});
		setAudios(data);
	}
	function stateaudio(e: any, index: any) {
		...
		var myAudio: any = document.getElementById(`audioId${index}`);
		return myAudio.paused ? myAudio.play() : myAudio.pause();
	}
	return (
		<div className="App">
			{audios.map((res: any, index: any) =>
				index !== 0
					? res && (
							<div
								style={{
