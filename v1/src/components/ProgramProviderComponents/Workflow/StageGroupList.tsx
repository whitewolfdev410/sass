...
const StageGroupList = ({ stages, onDeleteStage }: { stages: string[], onDeleteStage: any }) => {
			...
			<Box
				...
				<ButtonGroup variant="text" sx={{ minWidth: "100%" }} color="inherit">
					{stages?.map((stage, index) => (
						<>
							<Button sx={{px: 2, py: 0.5, maxWidth: "180px", minWidth: "100px"}}
									onMouseEnter={e => {
							...
							>
								{stage}
								{index === stageIndex && <Button style={style} sx={{color: "red"}} onClick={()=> {onDeleteStage && onDeleteStage(index)}}>Delete</Button>}
							</Button>
